...
| `arm_conditioning.py` | Conditioning on a 10q arm-level proxy, compared with unadjusted and gene-level conditioning (S3 Table) |
| `cliffs_delta.py` | Cliff's delta with bootstrap confidence intervals for the signature and each gene (Table 1, S2 Table) |
